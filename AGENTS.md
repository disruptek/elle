...
| `primitives` | Built-in functions; includes `port/path`, `string/size-of`, `with-traits`, `traits`, `sys/args` (returns args after `--`, empty without `--`), `sys/env` (returns env as struct with string keys; optional single-var lookup), `subprocess/exec`, `subprocess/wait`, `subprocess/kill`, `subprocess/pid`, `syntax-pair?`, `syntax-list?`, `syntax-symbol?`, `syntax-keyword?`, `syntax-nil?`, `syntax->list`, `syntax-first`, `syntax-rest`, `syntax-e`, `fiber/set-fuel`, `fiber/fuel`, `fiber/clear-fuel`, and `meta/origin` (returns source location of a closure as `{:file :line :col}`, or `nil`) |
