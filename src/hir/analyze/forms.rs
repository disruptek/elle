...
impl<'a> Analyzer<'a> {
    ...
    pub(crate) fn analyze_while(&mut self, items: &[Syntax], span: Span) -> Result<Hir, String> {
        ...
        let effect = cond.signal.combine(body.signal);
    ...
    pub(crate) fn analyze_eval(&mut self, items: &[Syntax], span: Span) -> Result<Hir, String> {
        ...
        let effect = Signal::yields().combine(expr.signal).combine(env.signal);
        Ok(Hir::new(
            HirKind::Eval {
                expr: Box::new(expr),
                env: Box::new(env),
    ...
    pub(crate) fn analyze_parameterize(
        ...
        for pair_syntax in bindings_syntax {
            ...
            effect = effect.combine(param.signal).combine(value.signal);
    ...
    pub(crate) fn analyze_cond(&mut self, items: &[Syntax], span: Span) -> Result<Hir, String> {
        ...
        for clause in &items[1..] {
            ...
            let body = self.analyze_body(&parts[1..], span.clone())?;
            effect = effect.combine(test.signal).combine(body.signal);
