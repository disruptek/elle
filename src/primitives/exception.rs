...
/// Throw an exception
pub fn prim_throw(args: &[Value]) -> Result<Value, String> {
    if args.is_empty() {
...
/// Create an exception
pub fn prim_exception(args: &[Value]) -> Result<Value, String> {
    if args.is_empty() {
...
pub fn prim_exception_message(args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
...
pub fn prim_exception_data(args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
