...
pub fn prim_define_c_union(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    ...
    let union_name = match &args[0] {
        ...
        _ => return Err("union name must be a string".to_string().into()),
    };
                ...
                match field_val {
                    Value::Cons(cons) => {
                        let field_name = match &cons.first {
                            ...
                            _ => return Err("field name must be a string".to_string().into()),
                        };

                        let field_type = match &cons.rest {
                            Value::Cons(rest_cons) => super::types::parse_ctype(&rest_cons.first)?,
                            _ => return Err("field must be (name type)".to_string().into()),
                        };
                    ...
                    _ => return Err("each field must be a cons cell".to_string().into()),
                }
            }
        }
        Value::Nil => {}
        _ => return Err("fields must be a list".to_string().into()),
    }

    if fields.is_empty() {
        return Err("union must have at least one field".to_string().into());
    }
