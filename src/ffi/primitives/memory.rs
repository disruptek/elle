...
pub fn prim_register_allocation(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 4 {
        return Err("register-allocation requires exactly 4 arguments".to_string().into());
    }

    let ptr = match &args[0] {
        Value::Int(id) => *id as *const std::ffi::c_void,
        _ => return Err("ptr must be an integer".to_string().into()),
    };

    let type_name = match &args[1] {
        Value::String(s) => s.as_ref(),
        _ => return Err("type-name must be a string".to_string().into()),
    };

    let size = match &args[2] {
        Value::Int(s) => *s as usize,
        _ => return Err("size must be an integer".to_string().into()),
    };
    ...
    let owner = match &args[3] {
        Value::String(s) => match s.as_ref() {
            ...
            _ => return Err("owner must be 'elle', 'c', or 'shared'".to_string().into()),
        },
        _ => return Err("owner must be a string".to_string().into()),
    };
...
pub fn prim_type_check(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("type-check requires exactly 2 arguments".to_string().into());
    }
...
pub fn prim_null_pointer(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.is_empty() {
        return Err("null-pointer? requires at least 1 argument".to_string().into());
    }
...
pub fn prim_with_ffi_safety_checks(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.is_empty() {
        return Err("with-ffi-safety-checks requires at least 1 argument".to_string().into());
    }
...
pub fn prim_type_check_wrapper(args: &[Value]) -> crate::error::LResult<Value> {
    if args.len() != 2 {
        return Err("type-check requires 2 arguments".to_string().into());
    }
