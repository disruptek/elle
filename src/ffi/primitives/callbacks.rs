...
pub fn prim_make_c_callback(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 3 {
        return Err("make-c-callback requires exactly 3 arguments".to_string().into());
    }
    ...
    let arg_types = match &args[1] {
        ...
        _ => return Err("arg-types must be a list".to_string().into()),
    };
...
pub fn prim_free_callback(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("free-callback requires exactly 1 argument".to_string().into());
    }

    let cb_id = match &args[0] {
        Value::Int(id) => *id as u32,
        _ => return Err("callback-id must be an integer".to_string().into()),
    };
