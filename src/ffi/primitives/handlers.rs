...
pub fn prim_define_custom_handler(vm: &VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 4 {
        return Err("define-custom-handler requires exactly 4 arguments".to_string().into());
    }

    let name = match &args[0] {
        Value::String(s) => s.as_ref().to_string(),
        _ => return Err("Handler name must be a string".to_string().into()),
    };

    let _priority = match &args[3] {
        Value::Int(p) => *p as i32,
        _ => return Err("Priority must be an integer".to_string().into()),
    };
...
pub fn prim_unregister_custom_handler(vm: &VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("unregister-custom-handler requires exactly 1 argument".to_string().into());
    }
...
pub fn prim_custom_handler_registered(vm: &VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("custom-handler-registered? requires exactly 1 argument".to_string().into());
    }
