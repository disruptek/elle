...
pub fn prim_load_header_with_lib(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("load-header-with-lib requires exactly 2 arguments".to_string().into());
    }

    let header_path = match &args[0] {
        Value::String(s) => s.as_ref(),
        _ => return Err("header-path must be a string".to_string().into()),
    };

    let lib_path = match &args[1] {
        Value::String(s) => s.as_ref(),
        _ => return Err("lib-path must be a string".to_string().into()),
    };
...
pub fn prim_define_enum(_vm: &mut VM, args: &[Value]) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("define-enum requires exactly 2 arguments".to_string().into());
    }

    let enum_name = match &args[0] {
        Value::String(s) => s.as_ref(),
        _ => return Err("enum name must be a string".to_string().into()),
    };
                ...
                match variant_val {
                    Value::Cons(cons) => {
                        let name = match &cons.first {
                            ...
                            _ => return Err("variant name must be a string".to_string().into()),
                        };

                        let value = match &cons.rest {
                            Value::Cons(rest_cons) => match &rest_cons.first {
                                Value::Int(n) => *n,
                                _ => return Err("variant value must be an integer".to_string().into()),
                            },
                            _ => return Err("variant must be (name value)".to_string().into()),
                        };

                        variants.push(EnumVariant { name, value });
                    }
                    _ => return Err("each variant must be a cons cell".to_string().into()),
                }
            }
        }
        Value::Nil => {}
        _ => return Err("variants must be a list".to_string().into()),
    }
